...
from app.service.common.subagents.stone_behaviors import register_stone_behaviors
...
class StoneAgentOrchestrator(AgentOrchestrator):
    ...
    def __init__(self, *, retrieval_service: RetrievalService | None = None) -> None:
        ...
        subagent_root = Path(__file__).resolve().parents[2] / "stone" / "subagents"
        all_subagents = load_markdown_subagents(subagent_root, self.behaviors)
        allowed = set(self.PIPELINE_AGENT_NAMES)
        self.subagents = [agent for agent in all_subagents if agent.name in allowed]
