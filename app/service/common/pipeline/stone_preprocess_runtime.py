...
from app.service.common.llm import OpenAICompatibleClient, parse_json_response
...
class StonePreprocessWorker:
    ...
    def cancel_project(self, project_id: str) -> bool:
        with self._lock:
            future_items = [
                (run_id, self._futures.get(run_id))
                for run_id, mapped_project_id in self._project_by_future.items()
                if mapped_project_id == project_id
            ]
        all_cancelled = True
        for run_id, future in future_items:
            if future is None:
                continue
