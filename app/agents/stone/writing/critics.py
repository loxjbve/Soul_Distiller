...
def _render_packet_critic_v3(payload: dict[str, Any]) -> str:
    ...
    if payload.get("defaulted_fields"):
        lines.extend(["", "通用兜底字段：", *[f"- {item}" for item in (payload.get("defaulted_fields") or [])[:6]]])
...
def _review_packet_v3(
    ...
    *,
            ...
            [
                ...
                },
                {
                    ...
                    "content": (
                        ...
                        "请特别检查：\n"
                        "1. 关键字段是不是只靠通用默认值在撑。\n"
                        "2. 题目相关的画像切片有没有真正进入 packet。\n"
                        "3. packet 有没有被压成过于规整的模板。\n"
                        "4. 是否存在可能让后续正文写成“像标签、不像作者”的风险。\n\n"
                        "返回 JSON：\n"
                        "{\n"
                        '  "pass": boolean,\n'
                        '  "score": number,\n'
                        ...
                        '  "matched_signals": ["命中的强项，简体中文"],\n'
                        '  "unsupported_fields": ["证据不足的关键字段，简体中文"],\n'
                        '  "defaulted_fields": ["仍然依赖通用兜底的字段，简体中文"],\n'
                        '  "overfit_risks": ["模板化或过拟合风险，简体中文"],\n'
                        '  "repair_instructions": ["如何修正写作包，简体中文"],\n'
                        '  "risks": ["剩余风险，简体中文"]\n'
                        "}"
                    ),
                },
    ...
    return _normalize_packet_critic_payload(payload if isinstance(payload, dict) else {}, writing_packet=writing_packet)
            ...
            [
                ...
                },
                {
                    ...
                    "content": (
                        ...
                        + (
                            "这是一轮改稿审判。你必须同时对照上一版成稿、当前修改稿和用户修改意见，判断这次改动是否真正响应了用户要求，同时不能破坏语言与逻辑。\n\n"
...
def _revision_request_adapter_v3(
    ...
    return {
        ...
        "judgment_mode": str(writing_packet.get("judgment_mode") or "").strip() or "通过贴身细节稳定判断",
...
def _revision_rerank_v3(source_trace: dict[str, Any], writing_packet: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "selection_reason": "沿用上一版写作包所绑定的证据集合。",
    ...
    blocks: list[dict[str, Any]] = [
        {
            ...
            "label": f"Stone v3 基线已就绪（{analysis_bundle.version_label}）",
